...
import { getSourceIndexStatusClassName, getSourceIndexStatusLabel } from '../helpers'
...
export function KnowledgeSourcesPanel({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3 p-4 lg:p-5">
      ...
      {shouldShowAddForm ? (
        <DashboardPanel className="shrink-0 p-4">
      ...
      <DashboardPanel className="min-h-0 flex-1 overflow-hidden">
        <div className="flex items-center justify-between border-b border-bf-border px-4 py-3 dark:border-slate-800">
          <DashboardSectionHeader title="Source list" detail={`${sources.filter(source => source.enabled).length} enabled`} />
        </div>
        ...
        <div className="min-h-0 overflow-y-auto">
          {loading ? (
            ...
            <div className="p-4">
              <div className="rounded-lg border border-dashed border-bf-border bg-bf-subtle px-5 py-8 text-center dark:border-slate-700 dark:bg-slate-950/30">
                ...
                <div className="mt-4 rounded-md border border-bf-border bg-bf-surface px-4 py-3 text-left text-[12px] text-bf-text dark:border-slate-800 dark:bg-slate-950/50 dark:text-slate-300">
            ...
            <div>
              <div className="grid grid-cols-[minmax(0,1.9fr)_minmax(9rem,1fr)_2.5rem] gap-2 border-b border-bf-border px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.12em] text-bf-muted dark:border-slate-800 dark:text-slate-400">
                <span>Source</span>
                <span>Status</span>
              ...
              <div className="divide-y divide-bf-border dark:divide-slate-800">
                {sources.map(source => {
                  ...
                  return (
                    <DashboardListRow key={source.id} className="grid grid-cols-[minmax(0,1.9fr)_minmax(9rem,1fr)_2.5rem] gap-2 px-4">
                      ...
                      <div className="min-w-0 text-[11px] text-bf-muted dark:text-slate-300">
                        <div className="truncate">
                          ...
                          <span className="mx-1 text-bf-border dark:text-slate-600">·</span>
                          <span>{source.id}</span>
                        </div>
                      ...
                      <details className="relative justify-self-end">
                        <summary className="flex h-8 w-8 cursor-pointer list-none items-center justify-center rounded-md border border-bf-border bg-bf-surface text-bf-muted transition-colors duration-150 hover:bg-bf-subtle hover:text-bf-text dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800">
                          <MoreHorizontal className="h-3.5 w-3.5" strokeWidth={1.8} />
                        </summary>
                        <div className="absolute right-0 top-10 z-10 w-44 overflow-hidden rounded-md border border-bf-border bg-bf-surface p-1 shadow-lg dark:border-slate-700 dark:bg-slate-900">
                          {actions.map(action => (
                            <button
                              key={action.label}
                              type="button"
                              ...
                              className="block w-full rounded-md px-3 py-2 text-left text-[13px] text-bf-text transition-colors duration-150 hover:bg-bf-subtle disabled:cursor-not-allowed disabled:opacity-40 dark:text-slate-200 dark:hover:bg-slate-800"
                            >
                              {action.label}
                            </button>
                          ))}
