...
Migration safety note: commands, ports, endpoints, and local runtime instructions in this historical document are reference-only and must not be used to stop, clean up, replace, or reconfigure Steve's current local BuildFlow setup during the Dokploy migration.
